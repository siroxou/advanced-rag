...
Admin-only. Aggregates the append-only ``audit_log`` (the metrics spine) into the
numbers the /metrics dashboard renders - deliberately P50/P95 latency rather than a
...
async def get_metrics(
    ...
    if "admin" not in user.roles:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="metrics are admin-only")
...
if __name__ == "__main__":  # ponytail self-check: window string flows into SQL
